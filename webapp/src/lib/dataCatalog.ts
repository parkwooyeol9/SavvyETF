...
export const DATA_CATALOG: DatasetSpec[] = [
  ...
  },
  {
    ...
    group: "news",
    kind: "timeseries",
    prefixes: ["nlp_history/", "nlp_history/days/", "nlp_history/headlines/"],
    hotWindow: "종목 JSON 점수 전 기간 + 최근 30일 제목 전체",
    archive: "일별 단면 nlp_history/days/{YYYY-MM-DD}.json 은 삭제하지 않음. 종목 JSON은 병합만 하고 짧은 시계열로 덮지 않음. 30일 이전 제목은 headlines/{code}/{YYYY-MM}.json",
    volatile: false,
