...
function downsample(points: AiGovPoint[], maxPoints: number): AiGovPoint[] {
...
async function fetchSignal(
  spec: (typeof AI_GOV_BUCKET_SPECS)[number]["signals"][number],
): Promise<AiGovSignal> {
  const base: AiGovSignal = {
    id: spec.id,
    symbol: spec.symbol,
    label: spec.label,
    thesis: spec.thesis,
...
export async function GET() {
  try {
    const buckets = await Promise.all(
      AI_GOV_BUCKET_SPECS.map(async (spec) => {
    ...
    return NextResponse.json(
      {
        ...
        note: "",
        buckets: [],
        error: exc instanceof Error ? exc.message : "ai-gov fetch failed",
      } satisfies AiGovPayload,
      { status: 500 },
    );
  }
}
